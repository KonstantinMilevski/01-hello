...
void line_render::set_pixel(const position& point, const color& c)
{
    ...
    // assert((index < buffer_size, "Too big index"));
    if (index >= buffer_size)
    {
        std::cerr << "Index is more then buffer_";
...
pixels line_render::pixels_position(const position& start, const position& end)
{
    pixels result;
    int    x0 = start.x;
    int    y0 = start.y;
...
void line_render::draw_line(const position& start, const position& end,
                            const color& c)
{
    pixels points_on_line = pixels_position(start, end);
