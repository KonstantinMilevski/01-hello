    ...
    std::vector<vertex>& out)
{
    ...
    {
        for (size_t p = 1; p != num_pixel_in_line + 1; ++p)
        {
    ...
    const vertex& single, const vertex& left, const vertex& right)
{
    std::vector<vertex> out;
    size_t              num_of_lines =
        static_cast<size_t>(std::round(std::abs(single.f0 - left.f0)));
    if (num_of_lines > 0)
    {
        for (size_t p = 1; p != num_of_lines + 1; ++p)
        {
            double t_index      = static_cast<double>(p) / (num_of_lines + 1);
                                                              ...
                                                              const vertex& v2)
{
    ...
    auto it_middle = std::find_if(
        ...
        [&](const position& pos) {
            return pos.y == static_cast<int32_t>(std::abs(middle.f1));
        });
